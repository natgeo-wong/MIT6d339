...
m1 = ChannelModel(0.5,1.,3.,11);  u1,Q1 = SolveChannelModel(m1)
m2 = ChannelModel(0.5,1.,3.,21);  u2,Q2 = SolveChannelModel(m2)
m3 = ChannelModel(0.5,1.,3.,41);  u3,Q3 = SolveChannelModel(m3)
m4 = ChannelModel(0.5,1.,3.,81);  u4,Q4 = SolveChannelModel(m4)
m5 = ChannelModel(0.5,1.,3.,161); u5,Q5 = SolveChannelModel(m5)
m6 = ChannelModel(0.5,1.,3.,321); u6,Q6 = SolveChannelModel(m6)
...
axs[6].plot([0,3],[-1,-7],lw=0.5,c="k",linestyle="--",label=L"|$\alpha$| = 2",legend="ur")
axs[6].format(xlim=(0,3),ylim=(-5.5,-2.5),xlabel="log N",ylabel=L"log $||u||_p$",ltitle="(f) Order of Convergence",suptitle="b = 1.0")

axs[3].colorbar(c1,loc="r",label="u")
axs[6].colorbar(c2,loc="r",label=L"u - $u_r$ / 10$^{-4}$")

fig.savefig(plotsdir("prj1_qn1f_b0d5.png"),transparent=false,dpi=250)