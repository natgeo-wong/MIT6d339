...
x = -2.5:0.01:2.5; nx = length(x)
y = -1.25:0.01:1.25; ny = length(y)
upa = zeros(nx,ny)
upn = zeros(nx,ny)
...
A,ψ,xc,xt = InteriorMatrices1(5); σn = A \ ψ
for iy = 1 : ny, ix = 1 : nx
    if (x[ix]^2 + y[iy]^2) <= 1
        upa[ix,iy] = upotanalyticqn2([x[ix],y[iy]])
...
A,ψ,xc,xt = InteriorMatrices1(10); σn = A \ ψ
for iy = 1 : ny, ix = 1 : nx
    if (x[ix]^2 + y[iy]^2) <= 1
        upa[ix,iy] = upotanalyticqn2([x[ix],y[iy]])
        upn[ix,iy] = upotnumeric(σn,xc,[x[ix],y[iy]])
...
A,ψ,xc,xt = InteriorMatrices1(20); σn = A \ ψ
for iy = 1 : ny, ix = 1 : nx
    if (x[ix]^2 + y[iy]^2) <= 1
        upa[ix,iy] = upotanalyticqn2([x[ix],y[iy]])
        upn[ix,iy] = upotnumeric(σn,xc,[x[ix],y[iy]])
...
fig.savefig(plotsdir("prj3_qn2b_field.png"),transparent=false,dpi=200)